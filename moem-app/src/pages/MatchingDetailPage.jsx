...
import {MOCK_POSTS,getDday} from './MatchingPage'
...
const MatchingDetailPage = () => {
    ...
    const post = useMemo(
      () => MOCK_POSTS.find((p) => String(p.id) === String(id)),
      [id]
    );
        ...
        <div className="container max-w-[1280px] mx-auto px-4 py-10">
          <div className="mb-6 flex items-center justify-between">
            ...
            <div className="flex items-center gap-2 text-xs text-zinc-500">
              <span className={`rounded-md px-2 py-0.5 font-semibold ${d.tone}`}>{d.label}</span>
              <span>
          ...
          <div className="mt-6 rounded-2xl border border-black/10 bg-white/60 p-6 shadow-sm backdrop-blur dark:border-white/10 dark:bg-zinc-900/60">
            ...
            <p className="mt-2 text-sm leading-7 text-zinc-700 dark:text-zinc-300">{post.desc}</p>
